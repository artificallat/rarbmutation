...
export default function WhatIs({ lang }: { lang: Lang }) {
  ...
  const refs = [
    {
      authors: "Srour et al. 2013",
      title: de
        ? "Rezessive und dominante Mutationen im Retinsäure-Rezeptor Beta bei Mikrophthalmie und Zwerchfellhernie"
        : "Recessive and Dominant Mutations in Retinoic Acid Receptor Beta in Cases with Microphthalmia and Diaphragmatic Hernia",
      href: "https://pmc.ncbi.nlm.nih.gov/articles/PMC3791254/",
    },
    {
      authors: "Srour et al. 2016",
      title: de
        ? "Gain-of-Function-Mutationen in RARB verursachen geistige Behinderung mit fortschreitender motorischer Beeinträchtigung"
        : "Gain-of-Function Mutations in RARB Cause Intellectual Disability with Progressive Motor Impairment",
      href: "https://onlinelibrary.wiley.com/doi/10.1002/humu.23004",
    },
    {
      authors: "Foster et al. 2019",
      title: de
        ? "RARB-assoziierte kolische Hypoganglionose"
        : "Retinoic acid receptor beta variant-related colonic hypoganglionosis",
      href: "https://onlinelibrary.wiley.com/doi/10.1002/ajmg.a.61078",
    },
    {
      authors: "Caron et al. 2023",
      title: de
        ? "Klinische und funktionelle Heterogenität bei Störungen des Retinsäure-Rezeptors Beta"
        : "Clinical and functional heterogeneity associated with the disruption of retinoic acid receptor beta",
      href: "https://www.gimjournal.org/article/S1098-3600(23)00869-9/fulltext",
    },
    {
      authors: "Trieschmann et al. 2023",
      title: de
        ? "De-novo-RARB-Variante in Verbindung mit Mikrophthalmie und Dystonie"
        : "De novo retinoic acid receptor beta (RARB) variant associated with microphthalmia and dystonia",
      href: "https://www.sciencedirect.com/science/article/abs/pii/S1769721223001088?via%3Dihub",
    },
    {
      authors: "Replogle et al. 2024",
      title: de
        ? "De-novo-nichtkodierende RARB-Variante bei komplexer Mikrophthalmie verändert ein putatives regulatorisches Element"
        : "A De Novo Noncoding RARB Variant Associated with Complex Microphthalmia Alters a Putative Regulatory Element",
      href: "https://onlinelibrary.wiley.com/doi/10.1155/2024/6619280",
    },
  ];
  ...
  return (
    <>
      ...
      {/* Quick stat strip */}
      <section className="container-tight -mt-4 pb-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {[
            { k: "2013", v: de ? "Erstbeschreibung" : "First described" },
            { k: "52", v: de ? "Fälle weltweit (2023)" : "Cases worldwide (2023)" },
            { k: "RARB", v: de ? "Verursachendes Gen" : "Causative gene" },
            { k: "0", v: de ? "Zugelassene Therapien" : "Approved therapies" },
          ].map(s => (
            <div key={s.k} className="bg-card border border-border rounded-2xl p-4 sm:p-5 text-center">
              <div className="text-3xl sm:text-4xl font-bold text-teal">{s.k}</div>
              <div className="text-xs sm:text-sm text-foreground/70 mt-1">{s.v}</div>
            </div>
          ))}
        </div>
      </section>

      {/* Overview narrative — broken into cards */}
      <section id="disease" className="container-tight pb-16">
        <div className="grid md:grid-cols-2 gap-5">
          <div className="bg-card border border-border rounded-2xl p-6 sm:p-7">
            <p className="text-xs uppercase tracking-[0.2em] text-teal font-semibold mb-3">{de ? "Ursache" : "Cause"}</p>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 sm:p-7">
            <p className="text-xs uppercase tracking-[0.2em] text-teal font-semibold mb-3">{de ? "Geschichte & Häufigkeit" : "History & prevalence"}</p>
            <p className="text-foreground/85 leading-relaxed">
              {de ? (
                <>Erstmals 2013 beschrieben (<Cite href="https://pmc.ncbi.nlm.nih.gov/articles/PMC3791254/">Srour et al. 2013</Cite>). Das systematische Review von 2023 fasste klinische Daten von nur 52 Personen weltweit zusammen (<Cite href="https://www.gimjournal.org/article/S1098-3600(23)00869-9/fulltext">Caron et al. 2023</Cite>). Die tatsächliche Zahl ist vermutlich höher — viele Kinder werden nicht oder falsch diagnostiziert.</>
              ) : (
                <>First described in 2013 (<Cite href="https://pmc.ncbi.nlm.nih.gov/articles/PMC3791254/">Srour et al. 2013</Cite>). The 2023 systematic review compiled clinical data from only 52 individuals worldwide (<Cite href="https://www.gimjournal.org/article/S1098-3600(23)00869-9/fulltext">Caron et al. 2023</Cite>). The true number is likely higher — many children are undiagnosed or misdiagnosed.</>
              )}
            </p>
          </div>

          <div className="bg-card border border-border rounded-2xl p-6 sm:p-7">
            <p className="text-xs uppercase tracking-[0.2em] text-teal font-semibold mb-3">{de ? "Klinisches Bild" : "Clinical picture"}</p>
            <p className="text-foreground/85 leading-relaxed">
              {de ? (
                <>Die meisten Kinder zeigen eine Kombination aus Augenfehlbildungen (z. B. Mikrophthalmie), niedrigem Muskeltonus (Hypotonie) und fortschreitenden Bewegungsstörungen (Dystonie und/oder Spastik) (<Cite href="https://www.sciencedirect.com/science/article/abs/pii/S1769721223001088?via%3Dihub">Trieschmann et al. 2023</Cite>). Manche haben zusätzlich Entwicklungsverzögerungen, Schluckprobleme oder Zwerchfell- bzw. Herzfehlbildungen. Die Ausprägung variiert stark.</>
              ) : (
                <>Most children show a combination of eye anomalies (e.g. microphthalmia), low muscle tone (hypotonia), and progressive movement disorders (dystonia and/or spasticity) (<Cite href="https://www.sciencedirect.com/science/article/abs/pii/S1769721223001088?via%3Dihub">Trieschmann et al. 2023</Cite>). Some also have developmental delay, swallowing difficulties or diaphragmatic / congenital heart defects. Severity varies widely.</>
              )}
            </p>
          </div>

          <div className="bg-card border border-border rounded-2xl p-6 sm:p-7">
            <p className="text-xs uppercase tracking-[0.2em] text-teal font-semibold mb-3">{de ? "Vererbung" : "Inheritance"}</p>
            <p className="text-foreground/85 leading-relaxed">
              {de ? (
                <>In der Regel autosomal-dominant: Eine veränderte Kopie des RARB-Gens reicht aus. Meist tritt die Variante neu beim Kind auf (de-novo), selten wird sie vererbt. Eine sehr seltene rezessive Form ist ebenfalls beschrieben (<Cite href="https://pmc.ncbi.nlm.nih.gov/articles/PMC3791254/">Srour et al. 2013</Cite>).</>
              ) : (
                <>Usually autosomal dominant: a single altered copy of RARB is enough. The variant most often appears de novo in the child; in a few families it is inherited. A very rare recessive form has also been reported (<Cite href="https://pmc.ncbi.nlm.nih.gov/articles/PMC3791254/">Srour et al. 2013</Cite>).</>
              )}
            </p>
          </div>
        </div>
      ...
      <section className="container-tight pb-16">
        <div className="bg-navy text-white rounded-3xl p-8 sm:p-10">
          <p className="text-xs uppercase tracking-[0.2em] text-amber font-semibold mb-6">
            {de ? "Fakten zu MCOPS12" : "Facts about MCOPS12"}
          </p>
          <ul className="grid sm:grid-cols-2 gap-4">
            {facts.map((f, i) => (
              <li key={i} className="flex gap-4 items-start bg-white/5 rounded-xl p-4 border border-white/10">
      ...
      <section className="container-tight py-16">
        ...
        <div className="grid gap-3">
          {refs.map(r => (
            <a key={r.href} href={r.href} target="_blank" rel="noreferrer" className="bg-card border border-border rounded-2xl p-5 flex items-start justify-between gap-4 hover:border-teal transition group">
              <div>
